...
public:
    ...
    Vector(const Vector& other)
        : data_(other.size_)
        , size_(other.size_)  {
        MoveOrCopyUninitialized(other.data_.GetAddress(), other.data_.GetAddress() + size_, data_.GetAddress());
    }
    ...
    void Reserve(size_t new_capacity) {
        ...
        MoveOrCopyUninitialized(data_.GetAddress(), data_.GetAddress() + size_, new_data.GetAddress());
        std::destroy_n(data_.GetAddress(), size_);
        data_.Swap(new_data);
    }
    ...
    iterator Emplace(const_iterator pos, Args&&... args) {
        ...
        if (std::distance(pos,cend()) == 0) {
            ...
            if (size_ == Capacity()) {
                ...
                MoveOrCopyUninitialized(begin(), const_cast<iterator>(pos), new_data.GetAddress());
                MoveOrCopyUninitialized(const_cast<iterator>(pos), end(), new_data.GetAddress() + cursor + 1);
                data_.Swap(new_data);
                std::destroy_n(new_data.GetAddress(), size_);
            }
            else {
                RawMemory<T> new_value(1);
                bool emplace_back = true;
                if (std::distance(pos, cend()) == 0) {
                    new (data_ + size_) T(std::forward<Args>(args) ...);
                }
                else {
...
private:
    template <typename iterator_v>
    void MoveOrCopyUninitialized(iterator_v start, iterator_v finish, iterator dest) {
        if constexpr (std::is_nothrow_move_constructible_v<T> || !std::is_copy_constructible_v<T>) {
            std::uninitialized_move(start, finish, dest);
        }
        else {
            std::uninitialized_copy(start, finish, dest);
        }
    }
